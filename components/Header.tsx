...
export default function Header({ sidebarCollapsed = false }: HeaderProps) {
  ...
  return (
    <header 
      className={`bg-white border-b border-gray-200/80 h-16 flex items-center justify-between px-4 lg:px-5 fixed top-0 right-0 z-30 backdrop-blur-sm transition-all duration-300 font-[var(--font-dm-sans)] ${
    ...
    >
      ...
      <div className="flex-1 flex justify-center px-4">
        <div className="relative group w-full max-w-md">
          <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 group-focus-within:text-green-600 transition-colors pointer-events-none z-10" />
          <input
            type="text"
            placeholder="Search equipment, requests..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full pl-10 pr-10 py-2 text-[13px] border border-gray-200 rounded-full bg-white shadow-sm hover:shadow-md focus:shadow-lg focus:outline-none focus:ring-2 focus:ring-green-600/20 focus:border-green-600 transition-all duration-200 text-gray-900 placeholder:text-gray-400"
          />
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 flex-shrink-0 ml-auto">
        ...
        <div className="relative flex items-center gap-3 pl-3 border-l border-gray-200" ref={userMenuRef}>
          ...
          >
            <div className="w-8 h-8 bg-gradient-to-br from-green-600 to-green-700 rounded-full flex items-center justify-center text-white font-medium text-sm shadow-sm">
              <User className="w-4 h-4" />
            </div>
            <div className="hidden lg:flex flex-col items-start">
              <span className="text-[13px] font-medium text-gray-900 leading-tight">John Doe</span>
              <span className="text-[11px] text-gray-500 leading-tight">Equipment Dept.</span>
            </div>
